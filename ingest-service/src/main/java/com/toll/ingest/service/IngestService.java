...
public class IngestService {
    ...
    *//**
 * Publishes a list of TagInfo events to Kafka.
 *//*
    public void publishTags(List<TagInfo> tags) {
        tags.forEach(tag->{
            // Ensure currentTrip exists
            if (tag.getCurrentTrip() == null) {
                throw new IllegalArgumentException("currentTrip is required on TagInfo");
            }
            // Compute toll
            double toll = rateService.getToll(tag.getCurrentTrip().getPlazaId(), tag.getCurrentTrip().getLaneId());
            tag.getCurrentTrip().setTollAmount(toll);

            // Produce event
            kafkaTemplate.send(topic, tag.getTagId(), tag);
            log.info("✅ Published tagId={} to topic={}", tag.getTagId(), topic);
        });
